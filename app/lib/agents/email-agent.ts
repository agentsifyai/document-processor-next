...
export class EmailAgent implements FileAgent {
  ...
  canHandle(file: File): boolean {
    ...
    return (
      this.emailExtensions.includes(extension) ||
  ...
  async analyze(file: File): Promise<AnalysisResult> {
    ...
    if (
      /invoice|receipt|payment|bill|statement|order|purchase/i.test(
        baseFileName
      )
    ) {
      ...
      /meeting|invite|calendar|schedule|appointment/i.test(baseFileName)
    ) {
    ...
    return {
      ...
      mimeType:
        file.type ||
        `application/${
          extension === "eml" ? "octet-stream" : "vnd.ms-outlook"
        }`,
      metadata: {
        inferredSubject,
        emailType,
