...
export class ImageAgent implements FileAgent {
  ...
  private readonly formatInfo: Record<string, {
    type: string;
    description: string;
    lossy?: boolean;
    features?: string[];
  ...
  private readonly imageCategories = [
    ...
    { regex: /background|texture|pattern|wallpaper/i, category: 'Background/Texture' },
  ...
  async analyze(file: File): Promise<AnalysisResult> {
    ...
    const baseFileName = file.name.replace(new RegExp(`\\.${extension}$`, 'i'), '');
    ...
    try {
      ...
      if (imageInfo) {
        ...
        else if (totalPixels > 2000000) resolution = 'medium resolution'; // > 2MP
        else if (totalPixels > 500000) resolution = 'standard resolution'; // > 0.5MP
      ...
      if (formatInfo.type === 'Vector') {
        ...
        description += `. ${resolution.charAt(0).toUpperCase() + resolution.slice(1)} image.`;
      }
      ...
      const dimensionInfo = imageInfo ? `${imageInfo.width}×${imageInfo.height} pixels` : '';
      ...
      return {
        ...
        metadata: {
          ...
          ...(imageInfo ? {
            ...
            resolution,
            megapixels: imageInfo ? ((imageInfo.width * imageInfo.height) / 1000000).toFixed(2) : undefined
          } : {}),
      ...
      return {
        ...
        metadata: {
          ...
          format: extension.toUpperCase(),
          features: formatInfo.features,
          contentDescription: `${imageCategory.toLowerCase()} file in ${formatInfo.description} format.`,
          analyzed: true
        }
      };
    }
  ...
  private getImageDimensions(file: File): Promise<{width: number, height: number} | null> {
    return new Promise((resolve) => {
      ...
      const browserViewableFormats = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp', 'svg', 'ico'];
